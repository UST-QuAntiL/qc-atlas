...
public interface PatternRelationTypeService {
     ...
     * <p>
     * The ID of the {@link PatternRelationType} parameter should be null, since the ID will be generated by the database when creating the entry. The
     * validation for this is done by the Controller layer, which will reject {@link PatternRelationType}s with a given ID in its create path.
     *
    ...
    /**
     * Find a database entry of a {@link PatternRelationType} that is already saved in the database. This search is based on the ID the database has
     * given the {@link PatternRelationType} object when it was created and first saved to the database.
     * <p>
    ...
    /**
     * Update an existing {@link PatternRelationType} database entry by saving the updated {@link PatternRelationType} object to the the database.
     * <p>
     * The ID of the {@link PatternRelationType} parameter has to be set to the ID of the database entry we want to update. The validation for this ID
     * to be set is done by the Controller layer, which will reject {@link PatternRelationType}s without a given ID in its update path. This ID will
     * be used to query the existing {@link PatternRelationType} entry we want to update. If no {@link PatternRelationType} entry with the given ID is
    ...
    /**
     * Delete an existing {@link PatternRelationType} entry from the database. This deletion is based on the ID the database has given the {@link
     * PatternRelationType} when it was created and first saved to the database.
     * <p>
