...
public interface SoftwarePlatformService {
    ...
    /**
     * Retrieve multiple {@link SoftwarePlatform} entries from the database where their name matches the name search parameter. If there are no
     * matches found an empty {@link Page} will be returned.
     * <p>
     ...
     * <p>
     * The ID of the {@link SoftwarePlatform} parameter should be null, since the ID will be generated by the database when creating the entry. The
     * validation for this is done by the Controller layer, which will reject {@link SoftwarePlatform}s with a given ID in its create path.
     *
    ...
    /**
     * Find a database entry of a {@link SoftwarePlatform} that is already saved in the database. This search is based on the ID the database has
     * given the {@link SoftwarePlatform} object when it was created and first saved to the database.
     * <p>
    ...
    /**
     * Update an existing {@link SoftwarePlatform} database entry by saving the updated {@link SoftwarePlatform} object to the the database.
     * <p>
     * The ID of the {@link SoftwarePlatform} parameter has to be set to the ID of the database entry we want to update. The validation for this ID to
     * be set is done by the Controller layer, which will reject {@link SoftwarePlatform}s without a given ID in its update path. This ID will be used
     * to query the existing {@link SoftwarePlatform} entry we want to update. If no {@link SoftwarePlatform} entry with the given ID is found this
    ...
    /**
     * Delete an existing {@link SoftwarePlatform} entry from the database. This deletion is based on the ID the database has given the {@link
     * SoftwarePlatform} when it was created and first saved to the database.
     * <p>
     * Objects that can be related to multiple {@link SoftwarePlatform}s will not be deleted. Only the reference to the deleted {@link
     * SoftwarePlatform} will be removed from these objects. These include {@link Implementation}s, {@link CloudService}s and {@link
     * ComputeResource}s.
     * <p>
    ...
    /**
     * Retrieve multiple {@link Implementation}s entries from the database of {@link Implementation}s that are linked to the given {@link
     * SoftwarePlatform}. If no entries are found an empty page is returned.
     * <p>
     * The amount of entries is based on the given {@link Pageable} parameter. If the {@link Pageable} is unpaged a {@link Page} with all entries is
     * queried.
     * <p>
     ...
     * <p>
     * If either the {@link SoftwarePlatform} or the {@link Implementation} with given IDs could not be found or if a database entry for both could be
