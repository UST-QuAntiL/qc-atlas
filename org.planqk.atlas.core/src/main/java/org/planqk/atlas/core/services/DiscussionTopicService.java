...
public interface DiscussionTopicService {
     ...
     * <p>
     * The ID of the {@link DiscussionTopic} parameter should be null, since the ID will be generated by the database
     * when creating the entry. The validation for this is done by the Controller layer, which will reject {@link
     * DiscussionTopic}s with a given ID in its create path.
     *
    ...
    /**
     * Find a database entry of a {@link DiscussionTopic} that is already saved in the database. This search is based on
     * the ID the database has given the {@link DiscussionTopic} object when it was created and first saved to the
     * database.
     * <p>
    ...
    /**
     * Update an existing {@link DiscussionTopic} database entry by saving the updated {@link DiscussionTopic} object to
     * the the database.
     * <p>
     * The ID of the {@link DiscussionTopic} parameter has to be set to the ID of the database entry we want to update.
     * The validation for this ID to be set is done by the Controller layer, which will reject {@link DiscussionTopic}s
     * without a given ID in its update path. This ID will be used to query the existing {@link DiscussionTopic} entry
     * we want to update. If no {@link DiscussionTopic} entry with the given ID is found this method will throw a {@link
    ...
    /**
     * Delete an existing {@link DiscussionTopic} entry from the database. This deletion is based on the ID the database
     * has given the {@link DiscussionTopic} when it was created and first saved to the database.
     * <p>
     * When deleting a {@link DiscussionTopic} all {@link org.planqk.atlas.core.model.DiscussionComment}s of the topic
     ...
     * <p>
     * If either the {@link DiscussionTopic} or the {@link org.planqk.atlas.core.model.KnowledgeArtifact} with given IDs
