...
public interface PublicationService {
     ...
     * <p>
     * The ID of the {@link Publication} parameter should be null, since the ID will be generated by the database when
     * creating the entry. The validation for this is done by the Controller layer, which will reject {@link
     * Publication}s with a given ID in its create path.
     *
    ...
    /**
     * Find a database entry of a {@link Publication} that is already saved in the database. This search is based on the
     * ID the database has given the {@link Publication} object when it was created and first saved to the database.
     * <p>
    ...
    /**
     * Update an existing {@link Publication} database entry by saving the updated {@link Publication} object to the the
     * database.
     * <p>
     * The ID of the {@link Publication} parameter has to be set to the ID of the database entry we want to update. The
     * validation for this ID to be set is done by the Controller layer, which will reject {@link Publication}s without
     * a given ID in its update path. This ID will be used to query the existing {@link Publication} entry we want to
     * update. If no {@link Publication} entry with the given ID is found this method will throw a {@link
    ...
    /**
     * Delete an existing {@link Publication} entry from the database. This deletion is based on the ID the database has
     * given the {@link Publication} when it was created and first saved to the database.
     * <p>
     * If no entry with the given ID is found this method will throw a {@link java.util.NoSuchElementException}.
     * <p>
     * Objects that can be related to multiple {@link Publication}s will not be deleted. Only the reference to the
     * deleted {@link Publication} will be removed from these objects. These include {@link Algorithm}s and  {@link
     * Implementation}s.
     *
    ...
    /**
     * <p>
     * Delete multiple existing {@link Publication} entry from the database. This deletion is based on the ID the
     * database has given the {@link Publication}s when they were created and first saved to the database.
     * <p>
     * If at least one entry with a given ID is not found this method will throw a {@link
     * java.util.NoSuchElementException}.
     *
    ...
    /**
     * Retrieve multiple {@link Algorithm}s entries from the database of {@link Algorithm}s that are linked to the given
     * {@link Publication} If no entries are found an empty page is returned.
     * <p>
     * The amount of entries is based on the given {@link Pageable} parameter. If the {@link Pageable} is unpaged a
     * {@link Page} with all entries is queried.
     * <p>
     * The given {@link Publication} is identified through its ID given as a parameter. If no {@link Publication} with
    ...
    /**
     * Retrieve multiple {@link Implementation}s entries from the database of {@link Implementation}s that are linked to
     * the given {@link Publication} If no entries are found an empty page is returned.
     * <p>
     * The amount of entries is based on the given {@link Pageable} parameter. If the {@link Pageable} is unpaged a
     * {@link Page} with all entries is queried.
     * <p>
     ...
     * <p>
     * If either the {@link Algorithm} or the {@link Publication} with given IDs could not be found or if a database
     ...
     * <p>
     * If either the {@link Implementation} or the {@link Publication} with given IDs could not be found or if a
     * database entry for both could be found but they are not linked a {@link java.util.NoSuchElementException} is
     * thrown.
     *
     * @param publicationId    The ID of the {@link Publication} we want to check
