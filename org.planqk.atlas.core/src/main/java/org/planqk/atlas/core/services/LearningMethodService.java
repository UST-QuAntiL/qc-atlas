...
public interface LearningMethodService {
     ...
     * <p>
     * The ID of the {@link LearningMethod} parameter should be null, since the ID will be generated by the database
     * when creating the entry. The validation for this is done by the Controller layer, which will reject {@link
     * LearningMethod}s with a given ID in its create path.
     *
     * @param learningMethod The {@link LearningMethod} object describing a type of learning that could be solved by an
     *                       {@link Algorithm}
    ...
    /**
     * Find a database entry of a {@link LearningMethod} that is already saved in the database. This search is based on
     * the ID the database has given the {@link LearningMethod} object when it was created and first saved to the
     * database.
     * <p>
    ...
    /**
     * Update an existing {@link LearningMethod} database entry by saving the updated {@link LearningMethod} object to
     * the the database.
     * <p>
     * The ID of the {@link LearningMethod} parameter has to be set to the ID of the database entry we want to update.
     * The validation for this ID to be set is done by the Controller layer, which will reject {@link LearningMethod}s
     * without a given ID in its update path. This ID will be used to query the existing {@link LearningMethod} entry we
     * want to update. If no {@link LearningMethod} entry with the given ID is found this method will throw a {@link
    ...
    /**
     * Delete an existing {@link LearningMethod} entry from the database. This deletion is based on the ID the database
     * has given the {@link LearningMethod} when it was created and first saved to the database.
     * <p>
     * All {@link LearningMethod}s where the deleted {@link LearningMethod} is a parent, will be have their parent
     * problem type set to null.
     * <p>
