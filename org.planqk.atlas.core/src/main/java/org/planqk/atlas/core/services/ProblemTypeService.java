...
public interface ProblemTypeService {
     ...
     * <p>
     * The ID of the {@link ProblemType} parameter should be null, since the ID will be generated by the database when creating the entry. The
     * validation for this is done by the Controller layer, which will reject {@link ProblemType}s with a given ID in its create path.
     *
     * @param problemType The {@link ProblemType} object describing a type of problem that could be solved by an {@link
    ...
    /**
     * Find a database entry of a {@link ProblemType} that is already saved in the database. This search is based on the ID the database has given the
     * {@link ProblemType} object when it was created and first saved to the database.
     * <p>
    ...
    /**
     * Update an existing {@link ProblemType} database entry by saving the updated {@link ProblemType} object to the the database.
     * <p>
     * The ID of the {@link ProblemType} parameter has to be set to the ID of the database entry we want to update. The validation for this ID to be
     * set is done by the Controller layer, which will reject {@link ProblemType}s without a given ID in its update path. This ID will be used to
     * query the existing {@link ProblemType} entry we want to update. If no {@link ProblemType} entry with the given ID is found this method will
    ...
    /**
     * Delete an existing {@link ProblemType} entry from the database. This deletion is based on the ID the database has given the {@link ProblemType}
     * when it was created and first saved to the database.
     * <p>
     * All {@link ProblemType}s where the deleted {@link ProblemType} is a parent, will be have their parent problem type set to null.
     * <p>
     * If no entry with the given ID is found this method will throw a {@link java.util.NoSuchElementException}.
     * <p>
     * If the {@link ProblemType} is still referenced by at least one {@link org.planqk.atlas.core.model.Algorithm} a {@link
