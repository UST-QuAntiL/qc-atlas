...
public interface AlgorithmService {
     ...
     * <p>
     * The ID of the {@link Algorithm} parameter should be null, since the ID will be generated by the database when creating the entry. The
     * validation for this is done by the Controller layer, which will reject {@link Algorithm}s with a given ID in its create path.
     *
     * @param algorithm The {@link Algorithm} object describing the properties of an algorithm that should be saved to the database
    ...
    /**
     * Find a database entry of a {@link Algorithm} that is already saved in the database. This search is based on the ID the database has given the
     * {@link Algorithm} object when it was created and first saved to the database.
     * <p>
     ...
     * <p>
     * The ID of the {@link Algorithm} parameter has to be set to the ID of the database entry we want to update. The validation for this ID to be set
     * is done by the Controller layer, which will reject {@link Algorithm}s without a given ID in its update path. This ID will be used to query the
     * existing {@link Algorithm} entry we want to update. If no {@link Algorithm} entry with the given ID is found this method will throw a {@link
    ...
    /**
     * Delete an existing {@link Algorithm} entry from the database. This deletion is based on the ID the database has given the {@link Algorithm}
     * when it was created and first saved to the database.
     * <p>
     * When deleting an {@link Algorithm} multiple objects referenced by the {@link Algorithm}  will be deleted together with it. All {@link
     * org.planqk.atlas.core.model.Implementation}s, {@link org.planqk.atlas.core.model.ComputeResourceProperty}s, {@link AlgorithmRelation}s, {@link
     * PatternRelation}s and {@link org.planqk.atlas.core.model.Sketch}es that are related to the {@link Algorithm}  will be deleted as well, since
     * they can only be related to one {@link Algorithm} at a time.
     * <p>
     * Objects that can be related to multiple {@link Algorithm}s will not be deleted. Only the reference to the deleted {@link Algorithm} will be
     * removed from these objects. These include {@link Publication}s, {@link ProblemType}s, {@link ApplicationArea}s and {@link
     * org.planqk.atlas.core.model.Tag}s.
     * <p>
    ...
    /**
     * Retrieve multiple {@link AlgorithmRelation}s entries from the database where the given {@link Algorithm} is either source or target algorithm.
    ...
    /**
     * Retrieve multiple {@link PatternRelation}s entries from the database where the pattern is related to the given {@link Algorithm}. If no entries
     * are found an empty page is returned.
     * <p>
     * The amount of entries is based on the given {@link Pageable} parameter. If the {@link Pageable} is unpaged a {@link Page} with all entries is
     * queried.
     * <p>
    ...
    /**
     * Retrieve multiple {@link Publication}s entries from the database of {@link Publication}s that are linked to the given {@link Algorithm}. If no
    ...
    /**
     * Retrieve multiple {@link ProblemType}s entries from the database where of {@link ProblemType}s that are linked to the given {@link Algorithm}.
     * If no entries are found an empty page is returned.
     * <p>
     * The amount of entries is based on the given {@link Pageable} parameter. If the {@link Pageable} is unpaged a {@link Page} with all entries is
     * queried.
    ...
    /**
     * Retrieve multiple {@link ApplicationArea}s entries from the database where of {@link ApplicationArea}s that are linked to the given {@link
     * Algorithm}. If no entries are found an empty page is returned.
     * <p>
     * The amount of entries is based on the given {@link Pageable} parameter. If the {@link Pageable} is unpaged a {@link Page} with all entries is
     * queried.
     * <p>
     ...
     * <p>
     * If either the {@link ProblemType} or the {@link Algorithm} with given IDs could not be found or if a database entry for both could be found but
     * they are not linked a {@link java.util.NoSuchElementException} is thrown.
     *
     * @param algorithmId   The ID of the {@link Algorithm} we want to check
     ...
     * <p>
     * If either the {@link ApplicationArea} or the {@link Algorithm} with given IDs could not be found or if a database entry for both could be found
