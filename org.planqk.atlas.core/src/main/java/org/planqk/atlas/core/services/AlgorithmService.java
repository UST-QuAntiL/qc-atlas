...
public interface AlgorithmService {
     ...
     * <p>
     * The ID of the {@link Algorithm} parameter should be null, since the ID will be generated by the database
    ...
    /**
     * Find a database entry of a {@link Algorithm} that is already saved in the database.
     * This search is based on the ID the database has given the {@link Algorithm}
    ...
    /**
     * Update an existing {@link Algorithm} database entry by saving the updated {@link Algorithm} object
     * to the the database.
     * <p>
     * The ID of the {@link Algorithm} parameter has to be set to the ID of the database entry we want to update.
     * The validation for this ID to be set is done by the Controller layer, which will reject {@link Algorithm}s
     * without a given ID in its update path.
     * This ID will be used to query the existing {@link Algorithm} entry we want to update.
     * If no {@link Algorithm} entry with the given ID is found this method will throw a
    ...
    /**
     * Delete an existing {@link Algorithm} entry from the database.
     * This deletion is based on the ID the database has given the {@link Algorithm}
     * when it was created and first saved to the database.
     * <p>
     * When deleting an {@link Algorithm} multiple objects referenced by the {@link Algorithm}  will be deleted together with it.
     * All {@link org.planqk.atlas.core.model.Implementation}s, {@link org.planqk.atlas.core.model.ComputeResourceProperty}s,
     * {@link AlgorithmRelation}s, {@link PatternRelation}s and {@link org.planqk.atlas.core.model.Sketch}es
     * that are related to the {@link Algorithm}  will be deleted as well, since they can only be related to one
     * {@link Algorithm} at a time.
     * <p>
    ...
    /**
     * Retrieve multiple {@link AlgorithmRelation}s entries from the database where the given {@link Algorithm} is either
     ...
     * <p>
     * If either the {@link Publication} or the {@link Algorithm} with given IDs could not be found or
     ...
     * <p>
     * If either the {@link ProblemType} or the {@link Algorithm} with given IDs could not be found or
     * if a database entry for both could be found but they are not linked a {@link java.util.NoSuchElementException}
     * is thrown.
     *
     * @param algorithmId The ID of the {@link Algorithm} we want to check
     ...
     * <p>
     * If either the {@link ApplicationArea} or the {@link Algorithm} with given IDs could not be found or
     * if a database entry for both could be found but they are not linked a {@link java.util.NoSuchElementException}
     * is thrown.
     *
     * @param algorithmId The ID of the {@link Algorithm} we want to check
