...
public interface SketchService {
    ...
    /**
     * Update an existing {@link Sketch} database entry by saving the updated {@link Sketch} object to the the database.
     * <p>
     * The ID of the {@link Sketch} parameter has to be set to the ID of the database entry we want to update. The validation for this ID to be set is
     * done by the Controller layer, which will reject {@link Sketch}s without a given ID in its update path. This ID will be used to query the
     * existing {@link Sketch} entry we want to update. If no {@link Sketch} entry with the given ID is found this method will throw a {@link
    ...
    /**
     * Delete an existing {@link Sketch} entry from the database. This deletion is based on the ID the database has given the {@link Sketch} when it
     * was created and first saved to the database.
     *
    ...
    /**
     * Find a database entry of a {@link Sketch} that is already saved in the database. This search is based on the ID the database has given the
     * {@link Sketch} object when it was created and first saved to the database.
     * <p>
    ...
    /**
     * Retrieve the image of a {@link Sketch} that is already saved in the database. This search is based on the ID the database has given the {@link
     * Sketch} object when it was created and first saved to the database.
     *
