...
public interface ImplementationService {
    ...
    /**
     * Creates a new database entry for a given {@link Implementation} that implements an existing {@link
     * org.planqk.atlas.core.model.Algorithm} and save it to the database.
     * <p>
     ...
     * Implementation}s with a given ID in its create path.
     * <p>
     * The {@link org.planqk.atlas.core.model.Algorithm} that is implemented will be queried from the database based on
     * the {@param implementedAlgorithmId} given to this method. If no {@link org.planqk.atlas.core.model.Algorithm}
     * with the given ID exist a {@link java.util.NoSuchElementException} will be thrown.
     *
     * @param implementation         The {@link Implementation} that should be saved to the database
     * @param implementedAlgorithmId The ID of the {@link org.planqk.atlas.core.model.Algorithm} this {@link
    ...
    /**
     * Find a database entry of a {@link Implementation} that is already saved in the database. This search is based on
     * the ID the database has given the {@link Implementation} object when it was created and first saved to the
     * database.
     * <p>
    ...
    /**
     * Update an existing {@link Implementation} database entry by saving the updated {@link Implementation} object to
     * the the database.
     * <p>
     ...
     * without a given ID in its update path. This ID will be used to query the existing {@link Implementation} entry we
     * want to update. If no {@link Implementation} entry with the given ID is found this method will throw a {@link
    ...
    /**
     * Delete an existing {@link Implementation} entry from the database. This deletion is based on the ID the database
     * has given the {@link Implementation} when it was created and first saved to the database.
     * <p>
     * When deleting an {@link Implementation} related {@link org.planqk.atlas.core.model.ComputeResourceProperty}s will
     * be deleted together with it.
     * <p>
     * Objects that can be related to multiple {@link Implementation}s will not be deleted. Only the reference to the
     * deleted {@link Implementation} will be removed from these objects. These include {@link Algorithm}s, {@link
     * Publication}s and  {@link SoftwarePlatform}s.
     * <p>
     ...
     * <p>
     * If either the {@link Implementation} or the {@link Algorithm} with given IDs could not be found or if a database
    ...
    /**
     * Retrieve multiple {@link Implementation}s entries from the database which implement a given {@link Algorithm}. If
     ...
     * linked to the given {@link Implementation} If no entries are found an empty page is returned.
     * <p>
     * The amount of entries is based on the given {@link Pageable} parameter. If the {@link Pageable} is unpaged a
     * {@link Page} with all entries is queried.
     * <p>
     * The given {@link SoftwarePlatform} is identified through its ID given as a parameter. If no {@link
     * SoftwarePlatform} with the given ID can be found a {@link java.util.NoSuchElementException} is thrown.
     *
     ...
     * given {@link Implementation} If no entries are found an empty page is returned.
     * <p>
     * The amount of entries is based on the given {@link Pageable} parameter. If the {@link Pageable} is unpaged a
     * {@link Page} with all entries is queried.
     * <p>
