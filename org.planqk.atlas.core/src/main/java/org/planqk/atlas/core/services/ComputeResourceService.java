...
public interface ComputeResourceService {
    ...
    /**
     * Retrieve multiple {@link ComputeResource} entries from the database where their name matches the name search parameter. If there are no matches
     ...
     * <p>
     * The ID of the {@link ComputeResource} parameter should be null, since the ID will be generated by the database when creating the entry. The
     * validation for this is done by the Controller layer, which will reject {@link ComputeResource}s with a given ID in its create path.
     *
    ...
    /**
     * Find a database entry of a {@link ComputeResource} that is already saved in the database. This search is based on the ID the database has given
     * the {@link ComputeResource} object when it was created and first saved to the database.
     * <p>
    ...
    /**
     * Update an existing {@link ComputeResource} database entry by saving the updated {@link ComputeResource} object to the the database.
     * <p>
     * The ID of the {@link ComputeResource} parameter has to be set to the ID of the database entry we want to update. The validation for this ID to
     * be set is done by the Controller layer, which will reject {@link ComputeResource}s without a given ID in its update path. This ID will be used
     * to query the existing {@link ComputeResource} entry we want to update. If no {@link ComputeResource} entry with the given ID is found this
    ...
    /**
     * Delete an existing {@link ComputeResource} entry from the database. This deletion is based on the ID the database has given the {@link
     * ComputeResource} when it was created and first saved to the database.
     * <p>
     * When deleting an {@link ComputeResource} related {@link org.planqk.atlas.core.model.ComputeResourceProperty}s will be deleted together with
     * it.
     * <p>
     * Objects that can be related to multiple {@link ComputeResource}s will not be deleted. Only the reference to the deleted {@link ComputeResource}
     * will be removed from these objects. These include {@link SoftwarePlatform}s and {@link CloudService}s.
     * <p>
     * If no entry with the given ID is found this method will throw a {@link java.util.NoSuchElementException}.
     * <p>
     * If the {@link ComputeResource} is still referenced by at least one {@link SoftwarePlatform} or {@link CloudService} a {@link
    ...
    /**
     * Retrieve multiple {@link CloudService}s entries from the database of {@link CloudService}s that are linked to the given {@link
     * ComputeResource}. If no entries are found an empty page is returned.
     * <p>
     * The amount of entries is based on the given {@link Pageable} parameter. If the {@link Pageable} is unpaged a {@link Page} with all entries is
     * queried.
     * <p>
     * The given {@link ComputeResource} is identified through its ID given as a parameter. If no {@link ComputeResource} with the given ID can be
